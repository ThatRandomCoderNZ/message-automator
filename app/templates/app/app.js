...
let message = new Vue({
    ...
    methods: {
        listenForSlash(event) {
            let containerElement = event.target;
            let caretPos = getCaretPosition(containerElement);
            
            let textNode;
            let plainText;
            if(caretPos[1] == 0){
                textNode = containerElement;
                plainText = stripSpans(getFirstRowText(containerElement.innerHTML));
            }else{
                textNode = containerElement.children[caretPos[1] - 1];
                plainText = stripSpans(textNode.textContent);
            }
            console.log(`Caret Position: X:${caretPos[0]}, Y:${caretPos[1]}`);
            let currentStroke = plainText.charAt(caretPos[0] - 1);
            let lastStroke = (caretPos[0] > 1) ? plainText.charAt(caretPos[0] - 2):"";

            if(currentStroke == "/" && lastStroke == "/"){
                console.log("Stroke Detected!");
                textNode.innerHTML = textNode.innerHTML.replace("//", "<span>Another Thing </span>");
            }
        }
    }
})

function getFirstRowText(allContent){
    let words = allContent.split("<div>");
    return words[0];
}
...
function getCaretPosition(editableDiv) {
    if (document.getSelection) {    // all browsers, except IE before version 9
        ...
        row = getCaretRow(sel);
        return [sel.getRangeAt(0).startOffset, row];
    } 
    else {
...
function getCaretRow(selection){
    let rowCount = 0;

    let parent = selection.anchorNode.parentElement;
    if(parent.id == "message-editor"){
        return 0;
    }
    
    let lastNode = selection.anchorNode.parentNode.previousSibling;
    while(lastNode != null){
